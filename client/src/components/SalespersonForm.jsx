...
const SkeletonLoader = ({ rows = 3 }) => (
  <div className="space-y-3 animate-pulse">
    {[...Array(rows)].map((_, i) => (
      <div
        key={i}
        className="bg-[var(--color-surface)] border border-[var(--color-border)] p-4 sm:p-5 rounded-2xl h-24 flex items-center justify-between gap-4"
      >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="text-lg sm:text-xl font-extrabold text-emerald-600">
          Deal Fully Settled & Cleared!
        </h3>
        <p className="text-xs sm:text-sm text-[var(--color-body)] leading-relaxed">
          All outstanding dues for{" "}
          <strong className="text-[var(--color-heading)]">
            {settledAlert.institute}
          </strong>{" "}
          have been paid in full. Balance is now{" "}
          <strong className="text-emerald-600">₹0 (Zero Due)</strong>. Both
          salesperson and account team have been notified successfully.
        </p>
        <button
          onClick={onClose}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="text-lg sm:text-xl font-extrabold text-[var(--color-heading)]">
          Confirm Logout
        </h3>
        <p className="text-xs sm:text-sm text-[var(--color-body)] leading-relaxed">
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-xs sm:text-sm text-[var(--color-body)] leading-relaxed">
          Are you sure you want to end your working day? Once ended, no further
          entries are allowed today.
        </p>
        <div className="flex gap-3 pt-2">
          <button
            type="button"
...
const [demoFollowUpTime, setDemoFollowUpTime] = useState("");
  ...
  const handleClearBroadcast = async (broadcastId, e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${API_BASE}/api/salesperson/broadcasts/${broadcastId}/dismiss`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      if (res.ok) {
        setBroadcastNotifications((prev) =>
          prev.filter((b) => b._id !== broadcastId),
        );
  ...
  const handleStartDay = async () => {
    ...
    if (isMocked) {
      setStatus({
        loading: false,
        success: "",
        error: "Mock location detected! Day start blocked.",
      });
      return;
    }
    ...
    try {
      ...
      setDayStatus("ACTIVE");
      setStartLocationName(
        data.startAddress || addressName || "GPS Location Recorded",
      );
  ...
  const executeEndDay = async () => {
    ...
    if (isMocked) {
      setStatus({
        loading: false,
        success: "",
        error: "Mock location detected! Action blocked.",
      });
      return;
    }
...
const fetchSalespersonNotifications = useCallback(async () => {
  ...
  const getSecureLocation = async () => {
    try {
      ...
      const isMocked =
        position.coords.mocked || position.coords.isFromMockProvider;
      if (isMocked) {
        toast.error(
          "⚠️ Security Warning: Mock Location (Fake GPS) app detected! Please disable fake GPS tools to record visits or submit invoices.",
        );
      ...
      console.warn(
        "Capacitor geolocation fallback to browser navigator:",
        err.message,
      );
  ...
  useEffect(() => {
    ...
    socketRef.current.on("connect", () => {
      console.log(
        "🔌 Salesperson Connected to Socket Server, ID:",
    ...
    socketRef.current.on("force_logout", (data) => {
      toast.error(
        data?.message || "Logged in from another device. Logging out...",
        { duration: 6000 },
      );
      localStorage.clear();
...
const [showPointsPopup, setShowPointsPopup] = useState(false);
  ...
  const citiesOfSelectedState = selectedStateCode
    ...
    : [];
  const citiesOfLeadState = leadStateCode
    ? City.getCitiesOfState("IN", leadStateCode)
    : [];
  ...
  useEffect(() => {
    ...
    const calculatedTotal =
      discountedSubtotal + gst + (formData.previousDueBalance || 0);
  ...
  const activeLeadsList = myLeads.filter(
    (lead) =>
      lead.leadStatus !== "Not Interested" && lead.leadStatus !== "Deal Close",
  );
  const totalLeadsCount = activeLeadsList.length;
  const approvedDealsCount = myDeals.filter(
    (d) => d.status === "approved",
  ).length;
  const pendingDealsCount = myDeals.filter(
    (d) => d.status === "pending",
  ).length;
  const totalPaidCollected = myDeals.reduce(
    (sum, d) => sum + (d.paidAmount || 0),
    0,
  );
...
const filteredLeads = myLeads.filter((lead) => {
        ...
        if (data[0] && data[0].Status === "Success") {
          ...
          if (details.District) {
            const poRes = await fetch(
              `https://api.postalpincode.in/postoffice/${details.District}`,
            );
            const poData = await poRes.json();
            if (poData[0] && poData[0].Status === "Success") {
              const pincodes = Array.from(
                new Set(poData[0].PostOffice.map((po) => po.Pincode)),
              );
  ...
  const handleLeadChange = (e) => {
    ...
    if (name === "mobileNo" && value.length >= 10) {
      const existingLead =
        myDeals.find((l) => l.mobileNo.trim() === value.trim()) ||
        myLeads.find((l) => l.mobileNo.trim() === value.trim());
      if (existingLead) {
        const matchedState = indianStates.find(
          (s) =>
            s.name.toLowerCase() === (existingLead.state || "").toLowerCase(),
        );
  ...
  const handleInvoiceCityChange = async (cName) => {
    ...
    try {
      const response = await fetch(
        `https://api.postalpincode.in/postoffice/${cName}`,
      );
      const data = await response.json();
      if (data[0]?.Status === "Success") {
        setAvailablePincodes(
          Array.from(new Set(data[0].PostOffice.map((po) => po.Pincode))),
        );
      }
    } catch (err) {
  ...
  const handleChange = (e) => {
    ...
    if (name === "instituteName") {
      ...
      const matchingDeal = myDeals.find(
        (d) =>
          d.instituteName && d.instituteName.trim().toLowerCase() === searchVal,
      );
      ...
      const matchedLead = activeLeadsList.find(
        (l) =>
          l.instituteName && l.instituteName.trim().toLowerCase() === searchVal,
      );
      ...
      if (targetData) {
        const matchedState = indianStates.find(
          (s) =>
            s.name.toLowerCase() === (targetData.state || "").toLowerCase(),
        );
        if (matchedState) setSelectedStateCode(matchedState.isoCode);

        const isExistingDealWithDue = Boolean(
          matchingDeal && matchingDeal.dueAmount > 0,
        );
        ...
        setFormData((prev) => ({
          ...
          categories:
            ...
            (targetData.category ? [targetData.category] : prev.categories),
  ...
  const handleAutoDetectLocation = async () => {
    ...
    if (isMocked) {
      setStatus({
        loading: false,
        success: "",
        error: "Mock location detected! Operation blocked.",
      });
      return;
    }
    ...
    if (!lat || !lng) {
      setStatus({
        loading: false,
        success: "",
        error: "Failed to retrieve GPS location or permission denied.",
      });
      return;
    }
    ...
    try {
      ...
      if (data && data.address) {
        ...
        if (fetchedCityName) {
          const poRes = await fetch(
            `https://api.postalpincode.in/postoffice/${fetchedCityName}`,
          );
          const poData = await poRes.json();
          if (poData[0] && poData[0].Status === "Success") {
            const pincodes = Array.from(
        ...
        setStatus({
          loading: false,
          success: "Location auto-detected securely and address filled!",
          error: "",
        });
      }
    } catch (err) {
    ...
    try {
      ...
      if (data && data.address) {
        ...
        setStatus({
          loading: false,
          success:
            "Location auto-detected securely and invoice address filled!",
          error: "",
        });
      }
    } catch (err) {
      setStatus({
        loading: false,
        success: "",
        error: "Failed to fetch invoice address.",
      });
    }
  };
...
const handleUpdateLeadStatus = async (
  ...
  const handleLeadSubmit = async (e) => {
    ...
    setStatus({
      loading: true,
      success: "Validating secure GPS & saving lead visit...",
      error: "",
    });
    ...
    if (isMocked) {
      setStatus({
        loading: false,
        success: "",
        error: "Action Blocked: Mock Location / Fake GPS detected!",
      });
      return;
    }
    ...
    const processLeadSubmission = async (latitude = null, longitude = null) => {
      ...
      try {
        ...
        setStatus({
          loading: false,
          success: resData.message || "Lead visit saved successfully!",
          error: "",
        });
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus({
      loading: true,
      success: "Verifying secure GPS & submitting installment...",
      error: "",
    });
    ...
    <div className="min-h-dvh bg-[var(--color-background)] p-3 sm:p-5 md:p-8 pb-24 md:pb-8 transition-colors duration-300">
      <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
        <AnimatePresence>
          {settledAlert && (
            <SettlementModal
              settledAlert={settledAlert}
              onClose={() => {
                setSettledAlert(null);
                setActiveView("dashboard");
              }}
            />
          )}
          ...
          {showEndDayModal && (
            <EndDayConfirmModal
              show={showEndDayModal}
              onClose={() => setShowEndDayModal(false)}
              onConfirm={executeEndDay}
            />
          )}
        </AnimatePresence>
...
<div className="grid grid-cols-2 gap-3 text-left text-xs sm:text-sm bg-[var(--color-surface)] p-4 rounded-2xl border border-[var(--color-border)] text-[var(--color-heading)]">
  <p>
    ⏱️ Start Time: <strong>{daySummaryModal.startTime || daySummaryModal.startedAt || "N/A"}</strong>
  </p>
  <p>
    🏁 End Time: <strong>{daySummaryModal.endTime || daySummaryModal.endedAt || "N/A"}</strong>
  </p>
  <p>
    ⏳ Working Hours: <strong>{daySummaryModal.workingHours || "0 hrs"}</strong>
  </p>
  <p>
    📍 Total Visits:{" "}
    <strong className="text-blue-600">
      {daySummaryModal.totalVisits ?? daySummaryModal.visitsCount ?? 0}
    </strong>
  </p>
  <p>
    💰 Collections:{" "}
    <strong className="text-emerald-600">
      ₹{(daySummaryModal.totalCollected ?? daySummaryModal.totalCollection ?? 0).toLocaleString("en-IN")}
    </strong>
  </p>
  <p>
    🛣️ Distance:{" "}
    <strong className="text-purple-600">
      {daySummaryModal.totalDistanceKm ?? daySummaryModal.distance ?? 0} KM
    </strong>
  </p>
</div>
      ...
      Logout
        ...
        <div className="bg-[var(--color-card)] border border-[var(--color-border)] p-4 sm:p-6 md:p-7 rounded-3xl shadow-sm flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
          <div className="flex items-start sm:items-center gap-3.5 w-full sm:w-auto min-w-0 flex-1">
            <span className="text-3xl p-3 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shrink-0">
              ⏰
            </span>
            <div className="min-w-0 flex-1">
              <h3 className="text-xs sm:text-sm font-bold text-[var(--color-heading)] uppercase tracking-wider">
              ...
              <p className="text-xs text-[var(--color-body)] mt-0.5 leading-relaxed">
                ...
                {dayStatus === "NOT_STARTED" &&
                  '⏳ Day not started. Click "Start Day" to capture GPS location and unlock leads & invoices.'}
                {dayStatus === "ACTIVE" &&
                  "🟢 Shift Active. GPS active and system entries are fully enabled."}
                {dayStatus === "ENDED" &&
                  "🔒 Shift Ended for today. Entries are locked until tomorrow."}
              </p>
              {startLocationName && (
                <p className="text-xs font-semibold text-[var(--color-primary)] mt-1.5 flex items-center gap-1.5 truncate">
                  📍 <span className="shrink-0">Start Location:</span>{" "}
                  <strong className="text-[var(--color-heading)] truncate">
                    {startLocationName}
                  </strong>
                </p>
              )}
            ...
            <button
              ...
              className={`flex-1 sm:flex-initial px-4 py-3 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 active:scale-95 min-h-[44px] ${
                activeView === "lead-form"
                  ? "bg-[var(--color-primary)] text-white shadow-md shadow-[var(--color-primary)]/20"
                  : "bg-[var(--color-surface)] text-[var(--color-heading)] border border-[var(--color-border)] hover:bg-[var(--color-border)]/50"
              }`}
            >
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div
                    ...
                    className="bg-[var(--color-card)] border border-[var(--color-border)] hover:border-[var(--color-primary)] p-5 sm:p-6 rounded-3xl shadow-sm cursor-pointer transition-all duration-300 hover:shadow-md hover:-translate-y-1 flex items-center justify-between group"
                  >
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-[var(--color-body)] mt-1">
                        Record client visit & meeting photo.
                      </p>
                    </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-[var(--color-body)] mt-1">
                        Manage leads across sales stages.
                      </p>
                    </div>
                    <span className="text-3xl p-3.5 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shrink-0 transition group-hover:scale-110">
                      📋
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                  <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-3xl p-4 sm:p-5 shadow-sm space-y-1.5 transition hover:shadow-md hover:-translate-y-0.5">
                    <span className="text-xs sm:text-sm text-[var(--color-body)] block font-medium">
                      Total Deals
                    </span>
                  ...
                  <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-3xl p-4 sm:p-5 shadow-sm space-y-1.5 transition hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <strong className="text-xl sm:text-2xl font-extrabold text-blue-600">
                      {totalLeadsCount}
                    </strong>
                  </div>
                  <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-3xl p-4 sm:p-5 shadow-sm space-y-1.5 transition hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <strong className="text-xl sm:text-2xl font-extrabold text-emerald-600">
                      {approvedDealsCount}
                    </strong>
                  </div>
                  <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-3xl p-4 sm:p-5 shadow-sm space-y-1.5 transition hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <strong className="text-xl sm:text-2xl font-extrabold text-amber-600">
                      {pendingDealsCount}
                    </strong>
                  </div>
                  ...
                  {loadingDeals ? (
                    ...
                    <div className="space-y-3.5">
                      {myDeals.map((deal) => (
                        <div
                          key={deal._id}
                          className="bg-[var(--color-surface)] border border-[var(--color-border)] p-4 sm:p-5 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-xs sm:text-sm transition hover:border-[var(--color-primary)]/40"
                        >
                          <div className="space-y-1.5 min-w-0 w-full md:w-auto">
                            <div className="flex flex-wrap items-center gap-2">
                              <strong className="text-sm sm:text-base text-[var(--color-heading)] font-bold break-words">
                                {deal.instituteName}
                              </strong>
                            ...
                            <p className="break-words">
                              👤 Contact:{" "}
                              <a
                                href={`tel:${deal.mobileNo}`}
                                className="text-[var(--color-primary)] font-bold hover:underline"
                              >
                                {deal.mobileNo}
                              </a>{" "}
                              | 📍 {deal.city || "N/A"}, {deal.state || ""}
                            </p>
                          ...
                          <div className="flex items-center justify-between md:justify-end gap-3 w-full md:w-auto border-t md:border-t-0 pt-3 md:pt-0 border-[var(--color-border)]">
                            <div className="text-left md:text-right">
                              ...
                              >
                                ₹{deal.dueAmount?.toLocaleString("en-IN")}{" "}
                                {deal.dueAmount === 0 ? "✨ (Cleared)" : ""}
                              </span>
                            </div>
            ...
            {activeView === "calendar" && (
              <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-3xl p-5 sm:p-6 md:p-8 shadow-sm space-y-4">
                <div className="border-b border-[var(--color-border)] pb-4">
                  <h3 className="text-base sm:text-lg font-bold text-[var(--color-heading)]">
                    📅 Upcoming Follow-ups & Meetings Schedule
                  </h3>
                  <p className="text-xs sm:text-sm text-[var(--color-body)] mt-1">
                    Keep track of all client callbacks and meetings scheduled
                    for upcoming dates.
                  </p>
                </div>
                ...
                {myLeads.filter(
                  ...
                  <div className="text-center py-16 bg-[var(--color-surface)] rounded-3xl text-xs sm:text-sm text-[var(--color-body)] space-y-3 border border-[var(--color-border)]">
                    ...
                    <button
                      ...
                      className="bg-[var(--color-primary)] text-white text-xs sm:text-sm px-5 py-3 rounded-xl active:scale-95 transition"
                    >
                  ...
                  <div className="space-y-3.5">
                    {myLeads
                      ...
                      .map((lead) => (
                        <div
                          key={lead._id}
                          className="bg-[var(--color-surface)] border border-[var(--color-border)] p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3.5 text-xs sm:text-sm transition hover:border-[var(--color-primary)]/40"
                        >
                          <div className="space-y-1.5 min-w-0 w-full sm:w-auto">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="bg-[var(--color-primary)] text-white px-3 py-1 rounded-full font-bold text-[11px] uppercase tracking-wider">
                                {lead.followUpAction || "Call"}
                              </span>
                              <strong className="text-sm sm:text-base text-[var(--color-heading)] font-bold break-words">
                                {lead.instituteName}
                              </strong>
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto border-t sm:border-t-0 pt-3 sm:pt-0 border-[var(--color-border)]">
                            ...
                            <div className="bg-[var(--color-card)] border border-[var(--color-border)] p-3.5 rounded-2xl text-right sm:min-w-[190px] shrink-0">
                              <span className="text-[var(--color-body)] block text-[11px] font-medium uppercase">
                                Scheduled For:
                              </span>
                              <strong className="text-emerald-600 text-xs sm:text-sm font-bold">
                                📅{" "}
                              ...
                              {lead.followUpTime && (
                                <span className="block text-[var(--color-heading)] font-semibold mt-0.5 text-xs">
                                  ⏰ {lead.followUpTime}
                                </span>
                              )}
                            </div>
        ...
        <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
          {callHistory.slice(0, 20).map((call) => (
            ...
            {activeView === "lead-form" && (
              <div>
                {status.success && (
                  <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 p-4 rounded-2xl mb-6 text-xs sm:text-sm font-semibold">
                    {status.success}
                  </div>
                )}
                {status.error && (
                  <div className="bg-red-500/10 border border-red-500/30 text-red-500 p-4 rounded-2xl mb-6 text-xs sm:text-sm font-semibold">
                    {status.error}
                  </div>
                )}
                ...
                <form
                  ...
                  className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-3xl p-5 sm:p-6 md:p-8 space-y-6 shadow-sm"
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-[var(--color-border)] pb-4">
                    <div>
                      <h3 className="text-xs sm:text-sm font-bold text-[var(--color-primary)] uppercase tracking-wider">
                        Record Visit / New Lead
                      </h3>
                      <p className="text-xs text-[var(--color-body)] mt-0.5">
                        Date and time are automatically recorded upon
                        submission.
                      </p>
                    </div>
                    <button
                      ...
                      className="bg-blue-500/10 hover:bg-blue-500/20 text-blue-600 border border-blue-500/25 text-xs sm:text-sm px-5 py-3 rounded-2xl font-bold transition cursor-pointer flex items-center gap-2 shrink-0 active:scale-95 min-h-[44px]"
                    >
                      📍 Locate Me
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 text-xs sm:text-sm">
                    <div>
                      <label className="block font-medium mb-2 text-[var(--color-heading)]">
                        Mobile Number (Auto-detects previous lead) *
                      </label>
                      <PhoneInput
                        country={"in"}
                        ...
                        onChange={(phone) => {
                          const cleanPhone = phone
                            ...
                            .slice(0, 12);
                          handleLeadChange({
                            target: { name: "mobileNo", value: cleanPhone },
                          });
                        }}
                        inputProps={{
                          name: "mobileNo",
                    ...
                    <div>
                      ...
                      <input
                        ...
                        name="instituteName"
                        required
                        value={leadFormData.instituteName}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-3.5 text-[var(--color-heading)] font-medium cursor-pointer"
                      >
                    ...
                    <div>
                      ...
                      {meetingPhotoPreview && (
                        <div className="mt-3">
                          <img
                            src={meetingPhotoPreview}
                            alt="Preview"
                            className="h-24 w-24 object-cover rounded-2xl border border-[var(--color-border)] shadow-sm"
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="md:col-span-2 p-4 sm:p-5 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl space-y-3.5">
                      <h4 className="text-xs sm:text-sm font-bold text-[var(--color-primary)] uppercase tracking-wider">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                        <div>
                          ...
                          <select
                            name="followUpAction"
                            value={leadFormData.followUpAction}
                            onChange={handleLeadChange}
                            className="w-full bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-3 text-xs sm:text-sm font-medium cursor-pointer"
                          >
                            <option value="Call">Call Back</option>
                            <option value="Next Meeting">
                              Meeting Reschedule
                            </option>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={handleLeadChange}
                            className="w-full bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-3 text-xs sm:text-sm cursor-pointer"
                          />
                        </div>
                        <div>
                          <label className="block font-medium mb-1.5 text-[var(--color-heading)]">
                          ...
                          <input
                            type="time"
                            name="followUpTime"
                            value={leadFormData.followUpTime}
                            onChange={handleLeadChange}
                            className="w-full bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-3 text-xs sm:text-sm cursor-pointer"
                          />
                        </div>
                  ...
                  <button
                    type="submit"
                    disabled={status.loading}
                    className="w-full bg-[var(--color-primary)] hover:opacity-90 text-white font-semibold py-4 rounded-2xl transition cursor-pointer disabled:opacity-50 shadow-sm text-xs sm:text-sm active:scale-95 min-h-[48px]"
                  >
                    {status.loading
                      ? "Detecting Secure GPS & Saving Visit..."
                      : "Save Lead Visit & Follow-up"}
                  </button>
                </form>
              </div>
            )}
              ...
              <div>
                ...
                >
                  <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-4">
                    <div>
                      <h3 className="text-xs sm:text-sm font-bold text-[var(--color-primary)] uppercase tracking-wider">
                        Step {invoiceStep} of 3
                      </h3>
                      <p className="text-xs sm:text-sm text-[var(--color-body)] mt-0.5">
                        {invoiceStep === 1 &&
                          "Client Information & Ledger Lookup"}
                        {invoiceStep === 2 &&
                          "Package Pricing, Add-ons & Coupons"}
                    ...
                    <div className="space-y-4 sm:space-y-5">
                      <div className="flex justify-between items-center">
                        <h4 className="text-xs sm:text-sm font-bold text-[var(--color-heading)] uppercase">
                          1. Client Details & Installment Ledger Lookup
                        </h4>
                        <button
                          type="button"
                          onClick={handleInvoiceAutoDetectLocation}
                          className="bg-blue-500/10 hover:bg-blue-500/20 text-blue-600 border border-blue-500/25 text-xs sm:text-sm px-5 py-2.5 rounded-2xl font-bold transition cursor-pointer flex items-center gap-2 min-h-[44px]"
                        >
                          📍 Locate Me
                        </button>
                      </div>
    ...
    {deal.instituteName} — Due: Rs. {deal.dueAmount?.toLocaleString("en-IN") || 0}
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-3.5 font-medium cursor-pointer"
                          >
                            <option value="">Select State</option>
                            {indianStates.map((st) => (
                              <option key={st.isoCode} value={st.isoCode}>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={formData.city}
                            onChange={(e) =>
                              handleInvoiceCityChange(e.target.value)
                            }
                        ...
                        <div className="md:col-span-2 p-4 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)]">
                          <label className="block font-bold text-[var(--color-heading)] mb-3 text-xs uppercase">
                            Package Validity Duration *
                          </label>
      ...
      >
        Next: Payment Mode ➔
                  ...
                  {invoiceStep === 3 && (
                    <div className="space-y-4 sm:space-y-5">
                      <h4 className="text-xs sm:text-sm font-bold text-[var(--color-heading)] uppercase">
                        3. Payment Mode & Proof Submission
                      </h4>
                      ...
                      <div className="p-4 sm:p-5 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl space-y-3.5">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs sm:text-sm">
                          {["ONLINE", "NEFT", "CASH", "CHEQUE"].map((mode) => (
                            <button
                              ...
                              className={`p-3.5 rounded-xl font-bold border transition cursor-pointer text-center min-h-[46px] ${
                                formData.paymentMode === mode
                                  ? "bg-[var(--color-primary)] text-white border-transparent shadow-sm"
                                  : "bg-[var(--color-card)] text-[var(--color-heading)] border-[var(--color-border)]"
                              }`}
                            >
                              {mode === "ONLINE"
                                ? "📱 Online / UPI"
                        ...
                        {(formData.paymentMode === "ONLINE" ||
                          ...
                          <input
                            ...
                            placeholder={
                              formData.paymentMode === "NEFT"
                                ? "Enter NEFT / Bank Reference Number..."
                        ...
                        {formData.paymentMode === "CASH" && (
                          <input
                            ...
                            className="w-full bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-3.5 text-xs sm:text-sm mt-3"
                          />
                        )}
                        {formData.paymentMode === "CHEQUE" && (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mt-3">
                            <input
                              ...
                              className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-3.5 font-mono text-xs sm:text-sm"
                            />
                            <input
                              type="text"
                              name="bankName"
                              ...
                              className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-3.5 text-xs sm:text-sm"
                            />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="p-4 sm:p-5 bg-[var(--color-surface)] p-4 rounded-2xl border border-[var(--color-border)] space-y-2.5 text-xs sm:text-sm">
                        <div className="flex justify-between">
                          <span>Grand Total (Bill + Past Dues):</span>
                          <strong>
                            ₹{formData.totalAmount.toLocaleString("en-IN")}
                          </strong>
                        </div>
                      ...
                      <div className="p-4 sm:p-5 bg-[var(--color-surface)] border border-dashed border-[var(--color-primary)]/40 rounded-2xl space-y-2.5">
                        <label className="block text-xs sm:text-sm font-semibold text-[var(--color-primary)]">
                          📸 Upload Payment Screenshot / Receipt Proof (.jpg,
                          .png) *
                        </label>
                        <input
                          type="file"
                          ...
                          className="block w-full text-xs sm:text-sm text-[var(--color-body)] cursor-pointer"
                        />
                      </div>
                      ...
                      <div className="flex justify-between pt-4">
                        ...
                        <button
                          type="submit"
                          disabled={status.loading}
                          className="bg-[var(--color-primary)] hover:opacity-90 text-white font-semibold px-8 py-3.5 rounded-2xl transition cursor-pointer disabled:opacity-50 shadow-sm text-xs sm:text-sm active:scale-95 min-h-[46px]"
                        >
                          {status.loading
                            ? "Submitting & Updating Ledger..."
