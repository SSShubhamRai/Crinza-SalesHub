...
const AccountantPanel = ({ userId, onLogout }) => {
  ...
  const [actionMessage, setActionMessage] = useState({ id: '', text: '', error: '' });
  ...
  const handleApprove = async (id) => {
    setActionMessage({ id, text: 'Generating PDF & sending email...', error: '' });
    try {
  ...
  const AccountantSkeleton = () => (
    <div className="space-y-4 animate-pulse">
      {[1, 2, 3].map((n) => (
        <div key={n} className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl p-6 flex flex-col md:flex-row justify-between gap-6 shadow-sm">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-[var(--color-border)] pb-5 gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-[var(--color-heading)] mt-1">Invoice Billing & Verification</h1>
            <p className="text-[var(--color-body)] text-xs">Logged in as: <strong className="text-[var(--color-primary)]">{userId}</strong></p>
          </div>

          <button onClick={onLogout} className="bg-red-500/10 hover:bg-red-500/20 text-red-600 border border-red-200 px-4 py-2 rounded-xl text-sm font-medium transition cursor-pointer">
            Logout
          </button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-[var(--color-card)] p-4 rounded-2xl border border-[var(--color-border)] shadow-sm">
          ...
          {activeTab === 'history' && (
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <span className="text-xs text-[var(--color-body)]">Filter Status:</span>
              <select
                value={statusFilter}
          ...
          <>
                ...
                {filteredPendingList.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {filteredPendingList.map((item) => (
                      <div key={item._id} className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl p-6 flex flex-col md:flex-row justify-between gap-6 shadow-sm">
                        
                        {/* Details Summary */}
                        <div className="space-y-2 flex-1">
                          <div className="flex flex-wrap items-center gap-3">
                            <span className="text-[var(--color-primary)] font-bold text-lg">{item.instituteName}</span>
                            <span className="text-xs bg-[var(--color-surface)] text-[var(--color-heading)] px-2.5 py-1 rounded-md border border-[var(--color-border)]">ID: {item.invoiceId}</span>
                            <span className="text-xs bg-[var(--color-primary-light)]/20 text-[var(--color-primary-dark)] px-2.5 py-1 rounded-md">Salesperson: {item.salespersonId}</span>
                            {item.couponCode && (
                              <span className="text-xs bg-emerald-500/10 text-emerald-600 px-2.5 py-1 rounded-md font-semibold border border-emerald-300">
                                🏷️ Coupon: {item.couponCode} (-₹{item.discountAmount || 999})
                              </span>
                            )}
                          </div>

                          <p className="text-sm text-[var(--color-heading)]">App: <strong>{item.appName}</strong> | Email: {item.email} | Mobile: {item.mobileNo}</p>
                          
                          <div className="flex gap-6 text-sm mt-2">
                            <span className="text-[var(--color-body)]">Total: <strong className="text-[var(--color-heading)]">₹{item.totalAmount}</strong></span>
                            <span className="text-[var(--color-success)]">Paid: <strong>₹{item.paidAmount}</strong></span>
                            <span className="text-[var(--color-danger)]">Due: <strong>₹{item.dueAmount}</strong></span>
                          </div>

                          {actionMessage.id === item._id && actionMessage.text && (
                            <div className="text-xs text-[var(--color-success)] mt-2 font-medium">{actionMessage.text}</div>
                          )}
                          {actionMessage.id === item._id && actionMessage.error && (
                            <div className="text-xs text-[var(--color-danger)] mt-2 font-medium">{actionMessage.error}</div>
                          )}
                        </div>
            ...
            {activeTab === 'history' && (
              <div className="space-y-4">
                <h3 className="text-lg font-bold text-[var(--color-heading)]">Processed Invoices History</h3>
                ...
                {filteredHistoryList.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {filteredHistoryList.map((item) => (
                      <div key={item._id} className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl p-5 shadow-sm space-y-3">
                        <div className="flex flex-wrap justify-between items-center gap-2 border-b border-[var(--color-border)] pb-2">
                          <div className="flex items-center gap-3">
                            <strong className="text-[var(--color-primary)] text-md">{item.instituteName} ({item.appName})</strong>
                            <span className="text-xs bg-[var(--color-surface)] px-2 py-0.5 rounded border border-[var(--color-border)] font-mono">
                              #{item.invoiceId}
                            </span>
                          </div>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase ${item.status === 'approved' ? 'bg-emerald-500/10 text-emerald-600 border border-emerald-300' : 'bg-red-500/10 text-red-500 border border-red-200'}`}>
                            {item.status}
                          </span>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 text-xs text-[var(--color-heading)]">
                          <div>📍 <strong>Address:</strong> {item.address || 'N/A'}, {item.city || ''}, {item.state || ''}</div>
                          <div>📞 <strong>Contact:</strong> {item.mobileNo} | {item.email}</div>
                          <div>👤 <strong>Salesperson:</strong> {item.salespersonId}</div>
                        </div>

                        <div className="flex flex-wrap justify-between items-center gap-2 pt-2 border-t border-[var(--color-border)] text-xs">
                          <div className="flex gap-4">
                            <span>Total: <strong>₹{item.totalAmount?.toLocaleString('en-IN')}</strong></span>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl max-w-lg w-full p-6 text-[var(--color-heading)] space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center border-b border-[var(--color-border)] pb-3">
                <h3 className="text-lg font-bold text-[var(--color-heading)]">Full Invoice Details</h3>
                <button onClick={() => setViewModalData(null)} className="text-[var(--color-body)] hover:text-[var(--color-heading)]">✕</button>
              </div>

              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><span className="text-[var(--color-body)] block text-xs">Institute</span> <strong>{viewModalData.instituteName}</strong></div>
                <div><span className="text-[var(--color-body)] block text-xs">App Name</span> <strong>{viewModalData.appName}</strong></div>
                <div><span className="text-[var(--color-body)] block text-xs">Mobile</span> <strong>{viewModalData.mobileNo}</strong></div>
                <div><span className="text-[var(--color-body)] block text-xs">Email</span> <strong>{viewModalData.email}</strong></div>
                <div><span className="text-[var(--color-body)] block text-xs">Pincode</span> <strong>{viewModalData.pincode}</strong></div>
                <div><span className="text-[var(--color-body)] block text-xs">GST No</span> <strong>{viewModalData.gstNo || 'N/A'}</strong></div>
                <div><span className="text-[var(--color-body)] block text-xs">Validity</span> <strong>{viewModalData.packageValidity}</strong></div>
                <div><span className="text-[var(--color-body)] block text-xs">Salesperson ID</span> <strong>{viewModalData.salespersonId}</strong></div>

                {/* Add-on Packages Display */}
                <div className="col-span-2 bg-[var(--color-surface)] p-3 rounded-xl border border-[var(--color-border)] mt-1">
                  <span className="text-[var(--color-body)] block text-xs mb-1 font-medium">Add-on Packages Selected</span>
                  <div className="flex flex-wrap gap-2 mt-1">
                    ...
                    {!viewModalData.addons?.testModule && !viewModalData.addons?.windowApp && !viewModalData.addons?.iosApp && (
                ...
                {viewModalData.couponCode && (
                  <div className="col-span-2 bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-xl text-xs flex justify-between items-center text-emerald-600">
                    <span>Applied Coupon Code: <strong>{viewModalData.couponCode}</strong></span>
                    <strong>Discount: -₹{viewModalData.discountAmount || 999}</strong>
                  </div>
                )}
              </div>
              ...
              <div className="bg-[var(--color-surface)] p-3 rounded-xl border border-[var(--color-border)] text-xs space-y-1.5">
                <div className="flex justify-between"><span>Base Amount:</span> <strong>₹{viewModalData.baseAmount || viewModalData.totalAmount}</strong></div>
                {viewModalData.discountAmount > 0 && (
                  <div className="flex justify-between text-emerald-600"><span>Coupon Discount:</span> <strong>-₹{viewModalData.discountAmount}</strong></div>
                )}
                <div className="flex justify-between border-t border-[var(--color-border)] pt-1 text-sm font-bold"><span>Total Amount:</span> <strong>₹{viewModalData.totalAmount}</strong></div>
                <div className="flex justify-between text-[var(--color-success)]"><span>Paid Amount:</span> <strong>₹{viewModalData.paidAmount}</strong></div>
                <div className="flex justify-between text-[var(--color-danger)] font-semibold"><span>Due Amount:</span> <strong>₹{viewModalData.dueAmount}</strong></div>
              </div>

              <div>
                <span className="text-[var(--color-body)] block text-xs mb-1">Terms & Conditions</span>
                <p className="text-xs bg-[var(--color-surface)] p-3 rounded-xl border border-[var(--color-border)] text-[var(--color-body)] whitespace-pre-line">{viewModalData.termsAndConditions}</p>
              </div>

              <button onClick={() => setViewModalData(null)} className="w-full bg-[var(--color-surface)] hover:bg-[var(--color-border)] py-2.5 rounded-xl font-medium text-sm border border-[var(--color-border)] cursor-pointer">Close</button>
            </div>
          </div>
        )}
        ...
        {editModalData && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <form onSubmit={handleEditSubmit} className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl max-w-xl w-full p-6 text-[var(--color-heading)] space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="flex justify-between items-center border-b border-[var(--color-border)] pb-3">
                <h3 className="text-lg font-bold text-[var(--color-heading)]">Edit Request Details</h3>
                <button type="button" onClick={() => setEditModalData(null)} className="text-[var(--color-body)] hover:text-[var(--color-heading)]">✕</button>
              </div>

              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <label className="text-xs text-[var(--color-body)]">Institute Name</label>
                  <input type="text" value={editModalData.instituteName} onChange={(e) => setEditModalData({ ...editModalData, instituteName: e.target.value })} className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-2 rounded-lg text-[var(--color-heading)]" />
                </div>
                <div>
                  <label className="text-xs text-[var(--color-body)]">App Name</label>
                  <input type="text" value={editModalData.appName} onChange={(e) => setEditModalData({ ...editModalData, appName: e.target.value })} className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-2 rounded-lg text-[var(--color-heading)]" />
                </div>
                <div>
                  <label className="text-xs text-[var(--color-body)]">Mobile Number</label>
                  <input type="text" value={editModalData.mobileNo} onChange={(e) => setEditModalData({ ...editModalData, mobileNo: e.target.value })} className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-2 rounded-lg text-[var(--color-heading)]" />
                </div>
                <div>
                  <label className="text-xs text-[var(--color-body)]">Email Address</label>
                  <input type="email" value={editModalData.email} onChange={(e) => setEditModalData({ ...editModalData, email: e.target.value })} className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-2 rounded-lg text-[var(--color-heading)]" />
                </div>

                <div>
                  <label className="text-xs text-[var(--color-body)]">Base Price (₹)</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="text-xs text-[var(--color-body)]">Coupon Code</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="text-xs text-[var(--color-body)]">Total Amount (₹) [Auto]</label>
                  <input 
                    type="number" 
                    readOnly 
                ...
                <div>
                  <label className="text-xs text-[var(--color-body)]">Paid Amount (₹)</label>
                  <input 
                    type="number" 
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setEditModalData(null)} className="flex-1 bg-[var(--color-surface)] py-2 rounded-xl text-sm border border-[var(--color-border)] cursor-pointer">Cancel</button>
                <button type="submit" className="flex-1 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] py-2 rounded-xl text-sm font-semibold text-white shadow-md cursor-pointer">Save Changes</button>
              </div>
            </form>
          </div>
        )}
        ...
        {rejectModalId && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl max-w-md w-full p-6 text-[var(--color-heading)] space-y-4 shadow-2xl">
              <h3 className="text-lg font-bold text-[var(--color-danger)]">Reject Invoice Request</h3>
              <p className="text-xs text-[var(--color-body)]">Specify the reason why this invoice is being rejected:</p>
              ...
              <div className="flex gap-3">
                <button onClick={() => setRejectModalId(null)} className="flex-1 bg-[var(--color-surface)] py-2.5 rounded-xl text-sm border border-[var(--color-border)] cursor-pointer">Cancel</button>
                <button onClick={handleRejectSubmit} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-semibold py-2.5 rounded-xl text-sm shadow-md cursor-pointer">Confirm Reject</button>
              </div>
            </div>
          </div>
        )}
